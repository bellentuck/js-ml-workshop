...
describe('Testing the basic KNN functionality.', function(){
	...
	//	typeA :
			// [
			// 	[
			// 		[ 0.3118912572245016, 0.7367498935760242 ], 1
			// 	],
			// 	[
			// 		[ 0.6283264385405443, 0.877470824915179 ], 1
			// 	]
			// 	//etc.
			// ]
		...
		describe('Optional helper functions to be used in predict and predictSingle', function(){
			...
			*/
			it('has function _sorted, which works as expected', function(){
				...
				expect(sorted[0]).to.equal(1) //[10,1]
				expect(sorted[1]).to.equal(1) //[20,1]
				expect(sorted[2]).to.equal(0) //[100,0]
				expect(sorted[3]).to.equal(0) //[110,0]
				expect(sorted[4]).to.equal(1) //[120,1]
			});
